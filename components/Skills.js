...
const Skills = () => {
    const skills = [' HTML/CSS', 'JavaSript', 'TypeScript', 'ReactJS', 'NextJS', 'TailwindCSS', 'React Native', 'C#', 'Unity'];
    return (
        <div className='max-w-[1240px] mx-auto relative'>
            <div
                ...
                {skills.map((skill, index) => <motion.h2 variants={fadeIn('down', 0.3)}
                    initial={'hidden'}
                    whileInView={'show'}
                    viewport={{ once: true, amount: 0.1 }} key={index} className='text-text1 text-center text-5xl font-black p-8 '>{skill}</motion.h2>)}
